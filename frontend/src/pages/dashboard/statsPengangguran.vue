...
export default {
  ...
  async mounted() {
    try {
      const totalResponse = await axios.get("http://localhost:5001/api/pengangguran/total");
      ...
      const totalRataRata = await axios.get("http://localhost:5001/api/pengangguran/rata-rata");
      ...
      const totalTinggiRendah = await axios.get("http://localhost:5001/api/pengangguran/tertinggi-terendah");
    ...
    <div class="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
      <div class="mx-auto max-w-3xl text-center">
        ...
        <p class="mt-4 text-gray-500 sm:text-xl">
          Statistik tahunan pengangguran terbuka berdasarkan data yang tersedia.
        </p>
      </div>

      <div class="mt-6">
        <h3 class="font-bold text-gray-900">TOTAL PENGANGGURAN TERBUKA PER TAHUN</h3>
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div v-for="(item, index) in dataTotal" :key="index" class="flex flex-col rounded-lg bg-blue-100 px-4 py-4 text-center">
            <h3 class="text-lg font-medium text-gray-500">
              Total Pengangguran Tahun {{ item._id }}
            </h3>
            <p class="text-2xl font-extrabold text-blue-600 ">
              {{ item.totalPengangguranTerbuka }}%
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="mt-6">
        <h3 class="font-bold text-gray-900">RATA-RATA PENGANGGURAN PER TAHUN</h3>
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
